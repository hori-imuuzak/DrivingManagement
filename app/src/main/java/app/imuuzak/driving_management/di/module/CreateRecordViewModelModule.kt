package app.imuuzak.driving_management.di.module

import androidx.lifecycle.ViewModel
import app.imuuzak.driving_management.di.ViewModelKey
import app.imuuzak.driving_management.ui.record.viewmodel.CreateRecordViewModel
import app.imuuzak.driving_management.ui.schedule.viewmodel.CreateTrackEventViewModel
import dagger.Binds
import dagger.Module
import dagger.multibindings.IntoMap

@Module
abstract class CreateRecordViewModelModule {
    @Binds
    @IntoMap
    @ViewModelKey(CreateRecordViewModel::class)
    abstract fun bindCreateRecordViewModel(vm: CreateRecordViewModel): ViewModel
}